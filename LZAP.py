...
#get key of item (idx) from dictionary
def get_string(idx):
    a=b''
    for i in dictionary:
        if dictionary[i]==idx:
...
#preparing input from chunks of bytes into integer
...
#compression proccess (inputting file, make dictionary, encoding, writing into files)
def compress():
    ...
    #WRITE FILE OUTPUT
    f=open(file=file_name+'.LZAP',mode='w+b')
    f.write(output)
    f.close()
    print("done!!")

#compression proccess (inputting file, make dictionary, decoding, writing into files)
def decompress():
    ...
    f=open(file=file_name+'_LZAP.txt',mode='w+b')
    f.write(output)
    f.close()
#main loop
while True:
